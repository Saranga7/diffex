        ...
        with torch.no_grad():
            ...
            while len(selected_coordinates) < self.M: # loop until we find M coordinates (or run out of images to explain)
                ...
                for i in tqdm.tqdm(range(0, remaining_images.shape[0], batch_size), leave=False):
                    ...
                    # Initialize the best sample indices and max_effect for each batch
                    batch_best_sample_idxs = []  
                    batch_max_effect = 0
                    batch_best_coordinate = None
                    batch_best_direction = None
